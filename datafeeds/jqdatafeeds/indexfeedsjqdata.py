...
# @File    : indexfeedsjqdata.py
# @Project : datafeeds
# @Software: PyCharm
# @Remark  : This is class of index market
...
class AIndexQuotationJqData(BaseJqData):
    LOGGER_NAME = "AIndexQuotationJqData"

    def __init__(self):
        super(AIndexQuotationJqData, self).__init__()
    ...
    def get_quotation(self, securityIds, items, frequency, begin_datetime, end_datetime, adjusted=None):
        ...
        if adjusted is not None:
            log.info("Index data has no adjust price, so we change adjusted %s to None" % adjusted)
            adjusted = None
