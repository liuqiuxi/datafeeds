...
class AOptionQuotationJqData(BaseJqData):
    LOGGER_NAME = "AOptionQuotationJqData"

    def __init__(self):
        super(AOptionQuotationJqData, self).__init__()
    ...
    def get_quotation(self, securityIds, items, frequency, begin_datetime, end_datetime, adjusted=None):
        ...
        if adjusted is not None:
            log.info("option data has no adjust price, so we change adjusted %s to None" % adjusted)
            adjusted = None
